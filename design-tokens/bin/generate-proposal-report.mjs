...
// the flag asset-class decision, accessibility, platform delivery, the
// promotion checklist and the open decisions.
...
const row = (entry) => {
  ...
  const pics = isFlag
    ? `${svg(f.color[24], 24)} ${svg(f.mono[24], 24)}`
...
<div class="cover">
  ...
  and a country-flag asset class — authored in the v4 design language, gated mechanically, and delivered as a review
  package (interactive decision catalog + PDF catalog + this report). Additive only: no shipped id changes.</p>
  <div class="kpis">
    ...
    <div class="kpi"><b>${c.icons * 24 + c.flags * 12}</b><span>generated SVGs</span></div>
  </div>
  <div class="strip">${["placeholder.avatar", "placeholder.card", "status.loading.dots", "status.offline", "utility.language", "plant.identify", "location.room", "calendar.add"]
    .filter((id) => registry.icons[id])
    .map((id) => svg(`svg/multicolor/${id}@24.svg`, 30))
    .join("")}${["flag.de", "flag.tr", "flag.gb", "flag.us", "flag.eu"].filter((id) => registry.flags[id]).map((id) => svg(`svg/color/${id}@24.svg`, 30)).join("")}</div>
...
<li><b>Country flags</b> (${groups[2].entries.length} ids) — a language/region picker needs them; the web client already carries
...
<tr><td>SVGs</td><td><code>svg/&lt;variant&gt;/&lt;id&gt;@&lt;size&gt;.svg</code> — ${c.icons * 24 + c.flags * 12} files</td></tr>
